	...
	public function pok_calculate_shipping( $package ) {
		...
		if ( ! empty( $rates ) ) {
			...
			foreach ( $rates as $rate ) {
				...
				if ( 'nusantara' === $this->setting->get( 'base_api' ) && 'yes' === $this->setting->get( 'specific_service' ) && ( ! isset( $rate['source'] ) || 'custom' !== $rate['source'] ) && apply_filters( 'pok_rates_apply_filter', true, $rate, $package ) ) {
				...
				$final_rate = apply_filters(
					'pok_rate', array(
						'id'        => 'pok-' . $rate['courier'] . '-' . sanitize_title( $rate['service'] ),
						'label'     => $label,
